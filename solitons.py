...
def copy_of_soliton(soliton, label='no_label'):
    ...
    Note that one cannot just use deepcopy, otherwise the branch points
    ...
    new_dashes = []
    for d in dashes:
        # NOTE: we don't pass the 'd.path', because that would be
...
def find_corresponding_pair(soliton, ref_gr_pair, multi=False):
    """
    For a given soliton, identifies which one(s) of its growing pairs
    correspond(s) to a certain 'reference pair'.
    """
